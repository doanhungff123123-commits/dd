return function()
	...
	-- ====== INSTANT PICKUP - BẤM E NHẶT NGAY KHÔNG CẦN CHỜ ======
	local function enableInstantPickup()
		...
		promptConn = ProximityPromptService.PromptShown:Connect(function(p)
			...
			pcall(function()
				-- Kiểm tra KHÔNG phải prompt mua Robux
				...
				if not isRobuxPrompt then
					-- BỎ THỜI GIAN CHỜ - SET VỀ 0
					p.HoldDuration = 0
					-- Đợi 1 frame
					task.wait()
					-- TỰ ĐỘNG TRIGGER (như bấm E)
	...
	local function run(points, direction)
		...
		enableNoclip(char)
		enableInstantPickup()  -- BẬT INSTANT PICKUP
		blockRobuxPrompts()
	...
	print("Script loaded! (Instant Pickup + Robux Block)")
end
